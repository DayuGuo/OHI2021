...
library(reshape2)
library(ggplot2)
data <- read_csv("IDI.csv")

data$region = as.factor(data$region)
data$region = ordered(data$region,levels = c('East Asia and Pacific','Europe and Central Asia',"Latin America and The Caribbean",
                                           'Middle East and North Africa','North America','South Asia','Sub-Saharan Arica'))

e.II = data[,c('region','Human Health','Animal Health','Environmental Health')]
